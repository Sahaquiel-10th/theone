...
export function PriceNotices({ api }: { api: typeof apiType }) {
  ...
  return <><button type="button" className="price-notice-banner" onClick={() => setOpen(true)}><strong>{status(latest)} · {latest.model}</strong><span>{latest.explanation}</span><small>{time(latest.effectiveAt || latest.publishedAt)} 生效 · 北京时间 · 查看详情</small></button>{open ? <SettingsDialog title="价格通知" onClose={() => setOpen(false)}><div className="price-notice-history">{items.slice((page - 1) * 5, page * 5).map(n => <article key={`${n.model}-${n.version}`}><strong>{n.model} · {status(n)}</strong><p className="price-notice-message">{n.explanation}</p><small>{time(n.effectiveAt || n.publishedAt)} 生效（北京时间） · v{n.version}</small></article>)}</div><div className="settings-pagination"><button type="button" disabled={page <= 1} onClick={() => setPage(page - 1)}>上一页</button><span>{page} / {Math.ceil(items.length / 5)}</span><button type="button" disabled={page * 5 >= items.length} onClick={() => setPage(page + 1)}>下一页</button></div></SettingsDialog> : null}</>;
}
