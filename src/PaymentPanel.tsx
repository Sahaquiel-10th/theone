...
function CurrentPrices({ api }: { api: typeof apiType }) {
  ...
  return <><p className="settings-caption">以下为实际费率，已包含优惠或服务费。计量单位由模型计算，不等于字数；每次实际消耗请查看账单。</p><input className="settings-search-input" aria-label="搜索模型价格" type="search" placeholder="搜索模型" value={query} onChange={e => { setQuery(e.target.value); setPage(1); }} /><div className="price-list">{filtered.slice((page - 1) * 6, page * 6).map(p => <article key={p.id}><strong>{p.name}</strong><span>{p.pricing?.label || "当前价格"}</span><p>{p.image !== undefined ? `${p.image} 电力 / 次` : `输入 ${p.input} / 输出 ${p.output} 电力 / 百万计量单位`}</p>{p.cachePrices ? <details><summary>缓存价格</summary><p>缓存读取 {p.cachePrices.read} / 基础写入 {p.cachePrices.write} / 1 小时写入 {p.cachePrices.write1h} 电力 / 百万计量单位</p></details> : null}</article>)}</div>{error || !prices || !filtered.length ? <p className="settings-empty">{error || (!prices ? "正在加载…" : "暂无匹配模型")}</p> : null}<Pagination page={page} total={filtered.length} size={6} onChange={setPage} /></>;
}
