...
export function preparePayment(db: Database, scope: { workspaceId: string; userId: string }, operationId: string, power: number, config: { appId: string; mchId: string }) {
  if (!/^[a-zA-Z0-9-]{16,80}$/.test(operationId) || !Number.isInteger(power) || power < 1 || power > 10000) throw new Error("请选择 1 至 10000 整数电力");
  ...
  if (existing) { if (existing.requestedMicros !== power * 1e6) throw new Error("订单重试金额不一致"); return { order: structuredClone(existing), created: false }; }
  const rate = db.settings.rechargeCnyPerPower, amountFen = Math.round(power * rate * 100);
  ...
  const order: RechargeOrder = { id, ...scope, requestedMicros: power * 1e6, amountCny: amountFen / 100, cnyPerPowerSnapshot: rate, status: "pending", createdAt: new Date().toISOString(),
    payment: { channel: "wechat", appId: config.appId, mchId: config.mchId, amountFen, expiresAt: new Date(Date.now() + 15 * 60000).toISOString(), state: "creating" } };
