...
app.post("/api/chat", auth(jwtSecret), asyncRoute(async (req, res) => {
  ...
  const latest = await store.read(); const memories = latest.userSavedMemories.filter((item) => item.workspaceId === req.workspaceId && item.userId === req.user!.id && item.status === "active").sort((a, b) => b.updatedAt.localeCompare(a.updatedAt)).slice(0, 10);
  ...
  const memoryContext = memories.length ? `【用户明确保存的记忆】\n${memories.map((item) => `- ${item.content}`).join("\n")}` : "";
...
app.patch("/api/memories/:id", auth(jwtSecret), asyncRoute(async (req, res) => { const content = requiredString(req.body.content, "记忆").slice(0, 500); const memory = await store.mutate((db) => { const target = db.userSavedMemories.find((item) => item.id === req.params.id && item.workspaceId === req.workspaceId && item.userId === req.user!.id && item.status === "active"); if (!target) throw new Error("记忆不存在"); target.content = content; target.updatedAt = now(); return target; }); res.json({ memory: { id: memory.id, text: memory.content, createdAt: memory.createdAt, updatedAt: memory.updatedAt } }); }));
app.delete("/api/memories/:id", auth(jwtSecret), asyncRoute(async (req, res) => { await store.mutate((db) => { const target = db.userSavedMemories.find((item) => item.id === req.params.id && item.workspaceId === req.workspaceId && item.userId === req.user!.id); if (!target) throw new Error("记忆不存在"); target.status = "deleted"; target.updatedAt = now(); }); res.json({ ok: true }); }));
