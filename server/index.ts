...
app.delete("/api/admin/models/:id/pricing/scheduled", ...admin, asyncRoute(async (req, res) => {
  await store.mutate(db => cancelScheduledPricing(db, String(req.params.id), req.user!.id)); res.json({ ok: true });
}));
