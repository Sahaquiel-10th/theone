...
test("billing HTTP pagination is owner-scoped and rejects a foreign payment before contacting WeChat", async t => {
  const db = { rechargeOrders: [ { id: "foreign", workspaceId: "w2", userId: "b", payment: { channel: "wechat" } } ],
    modelUsageRecords: Array.from({ length: 25 }, (_, i) => ({ id: String(i), workspaceId: i === 24 ? "w2" : "w", userId: i === 24 ? "b" : "a", createdAt: new Date(1000 + i).toISOString(), costMicros: 777, chargedMicros: 1 })), powerLedger: [] } as unknown as Database;
